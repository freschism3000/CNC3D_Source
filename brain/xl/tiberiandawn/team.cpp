...
bool TeamClass::Add(FootClass* obj, int typeindex)
{
    ...
    */
    if (obj->Mission == MISSION_STICKY || obj->Mission == MISSION_SLEEP || obj->Mission == MISSION_GUARD_AREA
        || obj->Mission == MISSION_HUNT || obj->Mission == MISSION_HARVEST) {
        return (false);
    }
    ...
    */
    while (unit) {
        ...
        if (unit->IsInitiated && !unit->IsInLimbo) {
            ...
            if (::Distance(Coord_Cell(unit->Center_Coord()), Center) > STRAY_DISTANCE /* XL: was Distance(CELL) */
                && (unit->Mission != MISSION_GUARD_AREA || !Target_Legal(unit->TarCom))) {
                if (unit->Mission != MISSION_MOVE || !Target_Legal(unit->NavCom)
                    || ::Distance(As_Cell(unit->NavCom), Center) > STRAY_DISTANCE) {
            ...
            while (unit) {
                ...
                if (unit->IsInitiated && !unit->IsInLimbo) {
                    ...
                    if (unit->What_Am_I() != RTTI_AIRCRAFT
                        && ::Distance(Coord_Cell(unit->Center_Coord()), Center) > STRAY_DISTANCE) { /* XL: was Distance(CELL) */
                        IsLagging = true;
                        finished = false;
                    } else {

                        if ((unit->Distance(Target) / ICON_LEPTON_W) > STRAY_DISTANCE
                            || (unit->What_Am_I() == RTTI_AIRCRAFT && ((AircraftClass*)unit)->Altitude > 0
    ...
    */
    while (unit) {
        ...
        if (!unit->IsInLimbo) {
            ...
            ** first unit.
            */
            if (::Distance(Coord_Cell(unit->Center_Coord()), ObjectiveCenter) > STRAY_DISTANCE) { /* XL: was Distance(CELL) */
                if (unit->Mission != MISSION_MOVE || !Target_Legal(unit->NavCom)
                    || ::Distance(As_Cell(unit->NavCom), ObjectiveCenter) > STRAY_DISTANCE) {
                    unit->Assign_Mission(MISSION_MOVE);
