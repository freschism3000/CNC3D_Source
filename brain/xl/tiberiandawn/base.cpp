...
void BaseClass::Read_INI(CCINIClass& ini)
{
    ...
    */
    for (int i = 0; i < count; i++) {
        ...
        */
        /*
        ** XL: the [Base] file format stores the CLASSIC packed 32-bit
        ** coordinate (X axis in the low 16 bits, Y in the high 16, each a
        ** 16-bit absolute lepton count). Axes are stride-independent, so the
        ** translation to the widened scalar is a straight per-axis unpack;
        ** Write_INI packs the same form back (axes MASKED to 16 bits -- fine
        ** for all classic content; an XL-native base beyond 256 cells per
        ** axis needs the Phase 2 format, DIVERGENCE.md formats), keeping the
        ** classic INI round-trip byte-identical.
        */
        unsigned long packed = (unsigned long)atol(strtok(NULL, ","));
        node.Coord = XY_Coord_Const((int)(packed & 0xFFFFUL), (int)((packed >> 16) & 0xFFFFUL));
...
void BaseClass::Write_INI(CCINIClass& ini)
{
    ...
    */
    for (int i = 0; i < Nodes.Count(); i++) {
        sprintf(uname, "%03d", i);
        /* XL: pack the classic 32-bit INI form back (see Read_INI above). */
        unsigned long packed = ((unsigned long)(Coord_Y(Nodes[i].Coord) & 0xFFFF) << 16)
                               | (unsigned long)(Coord_X(Nodes[i].Coord) & 0xFFFF);
        sprintf(buf, "%s,%lu", BuildingTypeClass::As_Reference(Nodes[i].Type).IniName, packed);
