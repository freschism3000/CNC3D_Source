...
{
    return (CELL)((coord & 0xFFFFFFFFULL) >> 8);
}

inline CELL Coord_YCell(COORDINATE coord)
{
    return (CELL)(coord >> 40);
}
...
**	COORD.CPP
*/
void Move_Point(int& x, int& y, register DirType dir, unsigned short distance);
...
**	FINDPATH.CPP
*/
// PathType * Find_Path(CELL source, CELL dest, FacingType *final_moves, int maxlen, int (*callback)(CELL, FacingType),
// int threshhold);
...
**	INI.CPP
*/
void Set_Scenario_Name(char* buf,
                       int scenario,
...
*/
/* XL: pixel-delta -> per-axis lepton delta, packed as a 64-bit relative
   coordinate. The classic form packed 16-bit axes with a shift; XY_Delta packs
   the identical per-axis values two's-complement in 32 bits, so negative pixel
   offsets keep working through Coord_Add. */
#define XYP_COORD(x, y) XY_Delta(((x)*ICON_LEPTON_W) / CELL_PIXEL_W, ((y)*ICON_LEPTON_H) / CELL_PIXEL_H)
...
{
    /* The classic bitfield assignment masked each axis to its field width and
       wrapped out-of-range values; the masks reproduce that exactly. */
    return (CELL)(((y & (MAP_CELL_H - 1)) << MAP_CELL_MAX_X_BITS) | (x & (MAP_CELL_W - 1)));
}
...
{
    return (int)(uint32_t)coord;
}
inline int Coord_Y(COORDINATE coord)
{
    return (int)(uint32_t)(coord >> 32);
}

inline int Cell_X(CELL cell)
{
    return (int)(cell & (MAP_CELL_W - 1));
}

inline int Cell_Y(CELL cell)
{
    return (int)((cell >> MAP_CELL_MAX_X_BITS) & (MAP_CELL_H - 1));
}
...
{
    return (CELL)((coord >> 32) & 0xFF);
}

inline COORDINATE Coord_Whole(COORDINATE coord)
{
    return (coord & COORD_KEEP_CELL);
}

inline COORDINATE Coord_Fraction(COORDINATE coord)
{
    return (coord & COORD_FRACTION_MASK);
}

inline COORDINATE Coord_Add(COORDINATE coord1, COORDINATE coord2)
{
    /* Two INDEPENDENT axis adds with no cross-axis carry -- exactly the classic
       per-axis 16-bit semantics widened to 32. Unsigned mod-2^32 addition is
       two's-complement wrap, so XY_Delta relative coordinates subtract by
       adding, as the old sign-extended shorts did. */
    uint32_t x = (uint32_t)coord1 + (uint32_t)coord2;
    uint32_t y = (uint32_t)(coord1 >> 32) + (uint32_t)(coord2 >> 32);
    return ((COORDINATE)y << 32) | x;
}
...
{
    return (coord & COORD_KEEP_CELL) | COORD_CENTER;
}

inline COORDINATE Coord_Mid(COORDINATE coord1, COORDINATE coord2)
{
    /* Positions cap at 2^20 leptons per axis, so the int sums are exact. */
    uint32_t x = (uint32_t)(((int)(uint32_t)coord1 + (int)(uint32_t)coord2) / 2);
    uint32_t y = (uint32_t)(((int)(uint32_t)(coord1 >> 32) + (int)(uint32_t)(coord2 >> 32)) / 2);
    return ((COORDINATE)y << 32) | x;
}

inline COORDINATE XYPixel_Coord(int x, int y)
{
    return XY_Delta((x * ICON_LEPTON_W) / ICON_PIXEL_W /*+LEPTON_OFFSET_X*/,
                    (y * ICON_LEPTON_H) / ICON_PIXEL_H /*+LEPTON_OFFSET_Y*/);
}
...
{
    return XY_Delta(Pixel_To_Lepton(x), Pixel_To_Lepton(y));
}

/*
**	Takes a old cell value (that assumes a map of 64x64) and adjusts it within
**  the enlarged map array. All old maps are aligned to the top left of the big
**  map. XL: generalized off the hard-coded 128 so the same confinement carries
**  64-wide content into the 1024 stride (docs/design-xl-brain.md).
*/
...
{
    return ((DirType)Desired_Facing256(Coord_X(coord1), Coord_Y(coord1), Coord_X(coord2), Coord_Y(coord2)));
}
...
{
    return ((DirType)Desired_Facing8(Coord_X(coord1), Coord_Y(coord1), Coord_X(coord2), Coord_Y(coord2)));
}
// inline int Direction16(COORDINATE coord1, COORD coord2) {return (Desired_Facing16(Coord_X(coord1), Coord_Y(coord1),
// Coord_X(coord2), Coord_Y(coord2)));}
inline DirType Direction(CELL cell1, CELL cell2)
{
    return (DirType)(Desired_Facing8(Cell_X(cell1), Cell_Y(cell1), Cell_X(cell2), Cell_Y(cell2)));
}
