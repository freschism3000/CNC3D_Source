...
COORDINATE AnimClass::Sort_Y(void) const
{
    Validate();
    if (Object) {
        return (Coord_Add(Object->Sort_Y(), XY_Delta(0, 1)));
    }
    if (Target_Legal(SortTarget)) {
        ObjectClass* obj = As_Object(SortTarget);
        if (obj && obj->IsActive) {
            return (Coord_Add(obj->Sort_Y(), XY_Delta(0, 1)));
        }
    }
...
short const* AnimClass::Overlap_List(void) const
{
    ...
    static short const OverlapAtom[] = {(-MAP_CELL_W * 2) - 1,
                                        (-MAP_CELL_W * 2),
                                        (-MAP_CELL_W * 2) + 1,
                                        (-MAP_CELL_W * 1) - 1,
                                        (-MAP_CELL_W * 1),
        ...
        if (Object) {
            ...
            */
            ...
            for (index = 0; index < Anims.Count(); index++) {
                if (Anims.Ptr(index) != this && Anims.Ptr(index)->Object == Object)
                    break;
            }
    ...
    */
    if (Class->IsFlameThrower) {
        ...
        COORDINATE c3 = Map.Closest_Free_Spot(
            Coord_Move(Center_Coord(), (DirType)((Class->Type - ANIM_FLAME_N) << 5), 0x0140), true);
