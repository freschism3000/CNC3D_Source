...
MEASURED. Emitter record layout: +0x00 next-in-chain (walked to NULL at RAM 0x800568BC), +0x04 a frame mask ANDed with s7+0xC0 so the emitter fires every 2^n frames, +0x08 a float velocity jitter, +0x0C a 0x40-byte PARTICLE NODE IMAGE. That last field is not a format of its own and never needed a decoder: the walker hands emitter+0x0C straight to RAM 0x8004E62C (ROM 0x057400 `jal`, delay slot `addiu $a1,$s1,0xc`), which allocates a 0x80-byte node and copies 0x40 bytes of the template into it VERBATIM, four words at a time, the loop bounded by `addiu $v1,$a1,0x40` at ROM 0x04F274, then does `lbu +0x30` / `sb +0x39` to arm the life counter. So the template IS the node that the field-by-field allocator at RAM 0x8004E6DC builds argument by argument, and that allocator's own stores give the layout: +0x04..+0x0C position, +0x10..+0x18 velocity, +0x1C..+0x24 acceleration, +0x28 size, +0x2C size growth per tick, +0x30 life in ticks (copied to +0x39), +0x31 alpha ramp slope, +0x32..+0x35 ARGB. Field for field that is the record `EfxSourceRec` in `game/efx_recipes.h` already models for the source table's visual sub-records. After the copy the walker translates the node's position by the draw command's own vec3 and adds `(rand8 - 0x7F) * jitter` to each velocity component. All 33 emitters are ONE CONTIGUOUS ARRAY of 0x4C-byte records, RAM 0x8009A980..0x8009B34C, reachable only from those chain heads, with no gaps and no record on two chains; every fire mask is 0, 1, 3 or 0x1F, so an emitter fires every frame, every 2nd, every 4th or every 32nd; and every chain feeds particle system 0, Intensity, materialised at ROM 0x04D430 as `lui $a0,0x800f; addiu $a0,$a0,-0x3e50` = RAM 0x800EC1B0. The values read as art rather than noise: SILO's damaged smoke is ARGB (255, 0, 255, 0), tiberium green, and the Power Plant's healthy emitter at RAM 0x8009AAB0 is a near-static pale-grey puff, ARGB (40, 200, 200, 200), size 13 growing 1.3 a tick over a 15-tick life, at offset [-85, 134, -107], which is the cooling-pond boil. The healthy chains are the smokestack plumes (WEAP 2 emitters, PROC 2, NUKE 1, NUK2 2, GUNBOAT 1) and the damaged chains are the damage smoke (TMPL 2, WEAP 2, PROC 2, NUKE 2, NUK2 2, and 1 each for EYE, GTWR, ATWR, OBLI, GUN, FACT, SILO, HPAD, HQ, SAM, AFLD, PYLE, HAND, FIX). HOSP, BIO and MISS have geometry but no emitter.
...
The `gl_gdi` precedent the requirement points at is the right one and it needs no extension: these are ordinary CI8 textures under the ordinary house TLUT, so the existing two-variant PackTex carries them unchanged.
...
- ANSWERED, and the answer was the opposite of what this line guessed: the emitter records at RAM 0x8009A980..0x8009B34C carry a template at +0x0C that IS the format already transcribed into game/efx_recipes.h, because the spawn path copies it into a particle node verbatim rather than parsing it. See the spawner section above for the four instructions. `tools/romdump/damage_art.py` now reads all 33 emitters and writes them into `tools/bakery/damage_art.json`, so the recipe is in the tree. What is still open is only the SCOPING: a damaged building that shows damage art and one that also smokes are separate builds, and the same table also carries the five HEALTHY chains (WEAP 2 emitters, PROC 2, NUKE 1, NUK2 2, GUNBOAT 1) that are health-independent and are what a reader means by the Power Plant's missing bubbling.
