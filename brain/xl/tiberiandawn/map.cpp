...
bool MapClass::Read_Binary_File(char const* fname, uint32_t* crc)
{
    ...
    */
    for (i = 0; i < OLD_MAP_CELL_TOTAL; i++) {
...
void MapClass::Logic(void)
{
    ...
    **
    **	XL: the scan GEOMETRY follows the scenario format, not the stride
    **	(DIVERGENCE.md, types). A classic-format scenario is confined content:
    **	the classic brain sweeps its own 128-wide, 16384-cell space in ~137
    **	ticks, and every growth/spread random draw the sweep makes is
    **	interleaved with the rest of the tick's draws by WHEN the sweep
    **	reaches the candidate cell. Sweeping the full 1024 stride instead
    **	would visit the same candidates at different ticks and shear the one
    **	seeded random stream against the classic brain on identical content
    ...
    */
    ...
    int subcount = scan_total / 136;
    int index;
    for (index = TiberiumScan; index < scan_total; index++) {
        int scanidx = index;
        if (!IsForwardScan)
            scanidx = (scan_total - 1) - index;
        CELL cell = scanidx;
        if (scan_confined)
            cell = (CELL)((scanidx % CLASSIC_MAP_CELL_W) + (scanidx / CLASSIC_MAP_CELL_W) * MAP_CELL_W);
