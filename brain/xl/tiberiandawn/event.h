...
public:
    ...
    */
    ...
    /*
    ** XL: CELL is int now, so the (RTTIType, CELL) and (int, CELL) ctors
    ** collapsed into their (RTTIType, int) and (TARGET, TARGET) twins. That
    ** is not a semantic loss: Data.Place aliases Data.Specific and
    ** Data.Special aliases Data.NavCom field-for-field (static_asserts in
    ** event.cpp), and Execute() always reads through the member its TYPE
    ** names -- the classic engine already leaned on exactly this pun when
    ** sidebar placement passed -1 through the int ctor for a PLACE event.
    */
