/* ================================================================================== *
 *  doscrate_mod.h -- the two bonus-crate overlays, from doscrate.pack.
 *
 *  The pack is game/bake_doscrate.py's output: WCRATE.SHP and SCRATE.SHP out of the
 *  shipped CONQUER.MIX, coloured through the palette that already sits inside
 *  dossidebar.pack, so the bake needs no 1995 CD mounted. Both are one 10x11 frame.
 *
...
static GLuint g_doscrateTex[2];        /* 0 = wood, 1 = steel -- the wire order */
static int    g_doscrateW[2], g_doscrateH[2];
static bool   g_doscrateHave = false;
...
static bool doscrate_load(const char* path)
{
    ...
    if (fread(&ver, 4, 1, f) != 1 || ver != 1 || fread(&count, 4, 1, f) != 1 || count != 2) {
        fprintf(stderr, "crates: %s: version %u with %u entries, expected 1 and 2\n",
                path, ver, count);
        fclose(f);
        return false;
    }
    ...
    fclose(f);
    g_doscrateHave = true;
    fprintf(stderr, "crates: %s: wood %dx%d, steel %dx%d\n", path,
...
{
    if (g_doscrateHave)
        glDeleteTextures(2, g_doscrateTex);
    g_doscrateHave = false;
}
