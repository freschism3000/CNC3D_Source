...
// PG_TO_FIX
    ...
    //------------------------------------------------------------------------
    my_sent += Send_Packets(net, multi_packet_buf, multi_packet_max, MPlayerMaxAhead, my_sent);
    ...
    //------------------------------------------------------------------------
    if (!Execute_DoList(MPlayerMax, HOUSE_MULTI1, net, NULL, their_frame, their_sent, their_recv)) {
        Stop_Game();
        return;
    }
                                    ...
                                    unsigned short* their_recv)
{
    ...
    while (true) {
        ...
        if (!dialog_timer.Time() && SpecialDialog == SDLG_NONE) {
            if (Process_Reconnect_Dialog(
                    &timeout_timer, their_frame, net->Num_Connections(), (first_time == 0), (reconnect_dlg == 0))) {
                return (RC_CANCEL);
            }
            reconnect_dlg = 1;
        }
        ...
        while ((messages_this_loop++ < 5) && net->Get_Private_Message(multi_packet_buf, &packetlen, &id)) {
        ...
        // Debug output
        //---------------------------------------------------------------------
        Print_Framesync_Values(Frame, MPlayerMaxAhead, net->Num_Connections(), their_recv, their_sent, my_sent);
        ...
        //.....................................................................
            ...
            if (Can_Advance(net, MPlayerMaxAhead, their_frame, their_sent, their_recv)) {
                break;
            }
        }
        ...
        if (MPlayerLocalID == MPlayerID[0]) {
            ...
            if (CommProtocol == COMM_PROTOCOL_MULTI_E_COMP) {
                ev.Data.FrameInfo.Delay = MAX(
                    ((((resp_time / 8) + (FrameSendRate - 1)) / FrameSendRate) * FrameSendRate), (FrameSendRate * 2));
                char flip[128];
...
static RetcodeType Process_Serial_Packet(char* multi_packet_buf, int first_time)
{
    ...
    if (serial_packet->Command == SERIAL_MESSAGE) {
        sprintf(txt, Text_String(TXT_FROM), serial_packet->Name, serial_packet->Message);
...
static int Handle_Timeout(ConnManClass* net, int* their_frame, unsigned short* their_sent, unsigned short* their_recv)
{
    ...
    //------------------------------------------------------------------------
        ...
        */
        if (MPlayerCount == 2 && GameToPlay == GAME_INTERNET && !GameStatisticsPacketSent) {
...
static int Build_Send_Packet(void* buf, int bufsize, int frame_delay, int num_cmds, int cap)
{
    ...
    //........................................................................
    if (CommProtocol == COMM_PROTOCOL_MULTI_E_COMP) {
        finfo->Frame = ((Frame + frame_delay + (FrameSendRate - 1)) / FrameSendRate) * FrameSendRate;
    } else {
...
static int Add_Compressed_Events(void* buf, int bufsize, int frame_delay, int size, int cap)
{
    ...
    while (OutList.Count && (num < cap)) {
        ...
        //.....................................................................
        if (CommProtocol == COMM_PROTOCOL_MULTI_E_COMP) {
            OutList.First().Frame = ((Frame + frame_delay + (FrameSendRate - 1)) / FrameSendRate) * FrameSendRate;
        } else {
            OutList.First().Frame = Frame + frame_delay;
        }
                          ...
                          unsigned short* their_recv)
{
    ...
    for (i = 0; i < MPlayerCount; i++) {
        ...
        for (j = 0; j < DoList.Count; j++) {
            ...
            //..................................................................
            if (DoList[j].MPlayerID == MPlayerID[i] && (unsigned)Frame >= DoList[j].Frame && !DoList[j].IsExecuted) {
...
static void Queue_Playback(void)
{
    ...
    // playback.
    //------------------------------------------------------------------------
    if (AllowAttract && Frame > 0 && (mx != Get_Mouse_X() || my != Get_Mouse_Y())) {
        GameActive = false;
        return;
    }
    mx = Get_Mouse_X();
...
static void Compute_Game_CRC(void)
{
    ...
    for (i = 0; i < Infantry.Count(); i++) {
        ...
        Add_CRC(&GameCRC, (int)(infp->Coord ^ (infp->Coord >> 32)) + (int)infp->PrimaryFacing.Current());
    }
    ...
    for (i = 0; i < Units.Count(); i++) {
        ...
        Add_CRC(&GameCRC,
                (int)(unitp->Coord ^ (unitp->Coord >> 32)) + (int)unitp->PrimaryFacing.Current() + (int)unitp->SecondaryFacing.Current());
    }
    ...
    for (i = 0; i < Buildings.Count(); i++) {
        ...
        Add_CRC(&GameCRC, (int)(bldgp->Coord ^ (bldgp->Coord >> 32)) + (int)bldgp->PrimaryFacing.Current());
    }
    ...
    for (i = 0; i < LAYER_COUNT; i++) {
        for (j = 0; j < Map.Layer[i].Count(); j++) {
            objp = Map.Layer[i][j];
            Add_CRC(&GameCRC, (int)(objp->Coord ^ (objp->Coord >> 32)) + (int)objp->What_Am_I()); /* XL: fold both axes */
        }
    }
...
void Print_CRCs(EventClass* ev)
{
    ...
    for (house = HOUSE_MULTI1; house <= HOUSE_MULTI6; ++house) {
        housep = HouseClass::As_Pointer(house);
        if (housep) {
            color = housep->RemapColor;
            snprintf(buffer,
                     sizeof(buffer),
                     "Multi%d: IsHuman:%d  Color:%s\n",
                     (int)house,
                     housep->IsHuman,
                     ColorNames[color]);
            fc.Write(buffer, strlen(buffer));
    ...
    for (house = HOUSE_MULTI1; house <= HOUSE_MULTI6; ++house) {
        if (HouseClass::As_Pointer(house)) {
            GameCRC = 0;
            snprintf(buffer, sizeof(buffer), "-------------------- MULTI%d Infantry -------------------\n", (int)house);
            fc.Write(buffer, strlen(buffer));
            for (i = 0; i < Infantry.Count(); i++) {
                infp = (InfantryClass*)Infantry.Active_Ptr(i);
                if (infp->Owner() == house) {
                    ...
                    snprintf(buffer,
                             sizeof(buffer),
                             "COORD:%llx   Facing:%d   Mission:%d   Type:%d\n",
                             infp->Coord,
                             (int)infp->PrimaryFacing.Current(),
                             infp->Get_Mission(),
                             infp->Class->Type);
                    fc.Write(buffer, strlen(buffer));
                }
            }
            Mono_Printf("Multi%d Infantry:%d\n", (int)house, GameCRC);
        }
    }
    ...
    for (house = HOUSE_MULTI1; house <= HOUSE_MULTI6; ++house) {
        if (HouseClass::As_Pointer(house)) {
            GameCRC = 0;
            snprintf(buffer, sizeof(buffer), "-------------------- MULTI%d Units -------------------\n", (int)house);
            fc.Write(buffer, strlen(buffer));
            for (i = 0; i < Units.Count(); i++) {
                unitp = (UnitClass*)Units.Active_Ptr(i);
                if (unitp->Owner() == house) {
                    Add_CRC(&GameCRC, (int)(unitp->Coord ^ (unitp->Coord >> 32)) + (int)unitp->PrimaryFacing.Current());
                    snprintf(buffer,
                             sizeof(buffer),
                             "COORD:%llx   Facing:%d   Facing2:%d   Mission:%d   Type:%d\n",
                             unitp->Coord,
                             (int)unitp->PrimaryFacing.Current(),
                             (int)unitp->SecondaryFacing.Current(),
                             unitp->Get_Mission(),
                             unitp->Class->Type);
                    fc.Write(buffer, strlen(buffer));
                }
            }
            Mono_Printf("Multi%d Units:%d\n", (int)house, GameCRC);
        }
    }
    ...
    for (house = HOUSE_MULTI1; house <= HOUSE_MULTI6; ++house) {
        if (HouseClass::As_Pointer(house)) {
            GameCRC = 0;
            snprintf(
                buffer, sizeof(buffer), "-------------------- MULTI%d Buildings -------------------\n", (int)house);
            fc.Write(buffer, strlen(buffer));
            for (i = 0; i < Buildings.Count(); i++) {
                bldgp = (BuildingClass*)Buildings.Active_Ptr(i);
                if (bldgp->Owner() == house) {
                    ...
                    snprintf(buffer,
                             ...
                             "COORD:%llx   Facing:%d   Mission:%d   Type:%d\n",
                             bldgp->Coord,
                             (int)bldgp->PrimaryFacing.Current(),
                             bldgp->Get_Mission(),
                             bldgp->Class->Type);
                    fc.Write(buffer, strlen(buffer));
                }
            }
            Mono_Printf("Multi%d Buildings:%d\n", (int)house, GameCRC);
        }
    }
    ...
    for (i = 0; i < LAYER_COUNT; i++) {
        ...
        for (j = 0; j < Map.Layer[i].Count(); j++) {
            objp = Map.Layer[i][j];
            Add_CRC(&GameCRC, (int)(objp->Coord ^ (objp->Coord >> 32)) + (int)objp->What_Am_I()); /* XL: fold both axes */
            snprintf(buffer, sizeof(buffer), "Object %d: %llx ", j, objp->Coord);
            fc.Write(buffer, strlen(buffer));
    ...
    for (i = 0; i < Logic.Count(); i++) {
        objp = Logic[i];
        Add_CRC(&GameCRC, (int)(objp->Coord ^ (objp->Coord >> 32)) + (int)objp->What_Am_I()); /* XL: fold both axes */
        snprintf(buffer, sizeof(buffer), "Object %d: %llx ", i, objp->Coord);
        fc.Write(buffer, strlen(buffer));
