...
public:
    ...
    */
    ...
    void Sight_From(HouseClass* house,
                    CELL cell,
                    int sightrange,
                    bool incremental = false); // Added house pointer parameter. ST - 3/6/2019 10:23AM
...
private:
    ...
    */
    /*
    ** XL: pinned at the classic brain's capacity (its MAP_CELL_W/2 = 64), NOT
    ** the XL stride's 512. The capacity is gameplay AND determinism surface:
    ** it decides when the overflow Random_Pick draws fire during the sweep,
    ** so scaling it with the stride would shear the random stream against the
    ** classic brain on identical confined content. Revisit as a TUNING number
    ** when XL-native content outgrows it (DIVERGENCE.md, types).
    */
    CELL TiberiumGrowth[TIBERIUM_LIST_MAX];
    ...
    */
    CELL TiberiumSpread[TIBERIUM_LIST_MAX];
