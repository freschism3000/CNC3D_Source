...
//	cell width.
//
//	XL: 10 bits -- ONE 1024-wide address space for the whole ladder
//	(docs/design-xl-brain.md, cellDesign), so 256, 512 and 1024 maps are
//	content tiers rather than repeated brain re-breaks. Smaller content lives
//	confined in the 1024 stride exactly as 64-wide content lived in the
//	classic MEGAMAPS 128 stride (Confine_Old_Cell, function.h).
//
//	HIGH_COORD_MASK is reborn with real slack: bits above 2^20 leptons
//	(4096 cells) per axis, so the Coord_Scatter/Coord_Move guards catch both
//	negative wrap and overrun in one AND -- and the classic trap where a
//	map exactly filling the coordinate type left the guard dead cannot
//	recur on this ladder.
#define MAP_CELL_MAX_X_BITS 10
#define MAP_CELL_MAX_Y_BITS 10
#define HIGH_COORD_MASK     0xFFF00000FFF00000ULL
...
// The legacy 64x64 scenario format's own geometry (the classic .BIN and the
// letter-waypoint namespace), used wherever old-format content is confined
// into the XL stride: Confine_Old_Cell and the old-format .BIN read/write
// loops. The classic MEGAMAPS build spelled this MAP_CELL_TOTAL/4, an
// identity that only held at a 128 stride.
#define OLD_MAP_CELL_W     64
#define OLD_MAP_CELL_TOTAL (OLD_MAP_CELL_W * OLD_MAP_CELL_W)

// The classic MEGAMAPS brain's own geometry (128-wide stride). The tiberium
// scan walks this window, at its chunk rate, for classic-format scenarios --
// see MapClass::Logic and DIVERGENCE.md (types): confined classic content
// must see the identical scan cadence the classic brain gives it, or the
// growth/spread random draws interleave differently and the cross-brain
// parity oracle (and any replay of classic content) shears.
#define CLASSIC_MAP_CELL_W     128
#define CLASSIC_MAP_CELL_TOTAL (CLASSIC_MAP_CELL_W * CLASSIC_MAP_CELL_W)

// The tiberium growth/spread candidate list capacity: the classic brain's own
// value (its MAP_CELL_W/2 with the 128 stride). Pinned rather than scaled --
// see MapClass and MapClass::Logic -- because overflow past it consumes the
// seeded random stream, making the capacity part of determinism, not just
// tuning.
#define TIBERIUM_LIST_MAX (CLASSIC_MAP_CELL_W / 2)
...
**	time.
*/
typedef enum MissionType : signed char
{
    MISSION_NONE = -1,
...
typedef enum PlayerColorType : signed char
{
    ...
    REMAP_GREY,  // Multi-Player house #7 (RA's grey scheme)
    REMAP_BROWN, // Multi-Player house #8 (RA's brown scheme)
...
*/
typedef enum DoType : signed char
{
...
**
**	XL (docs/design-xl-brain.md, coordDesign): COORDINATE is a 64-bit SCALAR,
**	X in the low uint32, Y in the high uint32. Each axis is an ABSOLUTE LEPTON
**	COUNT: cell-of-axis = axis >> 8, lepton-in-cell = axis & 0xFF, and
**	CELL_LEPTON_W stays 256 so every speed table and fixed-point quantum is
**	numerically unchanged from the classic engine. It stays a scalar on
**	purpose: !coord truth tests, == compares, union membership in EventClass,
**	memcpy'd saves and by-value calls all survive unchanged.
**
**	Per-axis arithmetic is two's-complement in 32 bits with NO cross-axis
**	carry, exactly the classic engine's per-axis 16-bit semantics widened;
**	relative coordinates (track tables, AdjacentCoord) are built by XY_Delta
**	below, never by hand-packed hex.
**
**	LEPTON_COMPOSITE and CELL_COMPOSITE are DELETED with the widening: the
**	axis sub-bytes are shifts now (function.h), and CELL is a plain int32
**	flat index into the MAP_CELL_W-stride cell array.
*/
#include <stdint.h>

#ifdef __BIG_ENDIAN__
#error "the XL brain is little-endian only (the classic brain's big-endian arms died with the composite unions)"
#endif

typedef uint32_t LEPTON;
...
/*
**	The coordinate bit-idiom constants, reborn at 64 bits. The classic engine
**	wrote these as raw packed hex (0x00800080 and family, ~164 sites); the XL
**	brain names them once and builds every relative offset through XY_Delta.
*/
#define COORD_CENTER        0x0000008000000080ULL /* both axes' lepton bytes at cell centre */
#define COORD_KEEP_CELL     0xFFFFFF00FFFFFF00ULL /* strip fractions, keep whole cells      */
#define COORD_FRACTION_MASK 0x000000FF000000FFULL /* keep fractions, strip whole cells      */

/*
**	XY_Delta -- a RELATIVE coordinate: per-axis two's-complement deltas packed
**	into the scalar, so Coord_Add's independent 32-bit axis adds wrap exactly
**	as the classic engine's 16-bit axis adds did. XY_Coord_Const is the same
**	packing for ABSOLUTE compile-time coordinates (stopping positions and the
**	like); both exist so no table ever hand-packs hex again.
*/
constexpr COORDINATE XY_Delta(int dx, int dy)
{
    return ((COORDINATE)(uint32_t)dy << 32) | (uint32_t)dx;
}
constexpr COORDINATE XY_Coord_Const(int x, int y)
{
    return ((COORDINATE)(uint32_t)y << 32) | (uint32_t)x;
}
...
#define HOUSE_MAX (HOUSE_COUNT + 1) // Lasts entire scenario. Derived the way REDALERT does it.
...
typedef enum WindowNumberType : unsigned char
{
    ...
    WINDOW_VIRTUAL // Window that is used when we want to capture rendering information without actually rendering. ST -
                   // 1/15/2019 3:03PM
} WindowNumberType;
...
*/
typedef enum FacingType : signed char
{
    FACING_NONE = -1,
    FACING_N,  // North
...
*/
typedef enum SpeedType : signed char
{
    SPEED_NONE = -1,
...
/*
**	EIGHTPLAYERS: the eight-player roster fits its wire and score formats with ZERO
