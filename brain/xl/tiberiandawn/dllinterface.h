...
{
    bool PassesProximityCheck; // If the structure was placed in this cell, does that satisfy the proximity check for
                               // the whole structure?
...
struct CNCSidebarEntryStruct
{
    ...
    bool BuildableViaCapture; // Is this buildable due to the capture of a structure of a different faction. This will
                              // be false for captured structures of the same faction (ActLike)
...
struct CNCSidebarStruct
{
    ...
    unsigned int
        TotalHarvestedCredits; // Complete total of gained credits over the match (does not include starting credits)
...
// EIGHTPLAYERS: the game-over stats block tracks exactly eight players -- the full
// roster with zero headroom. Widening MAX_PLAYERS past this silently drops players
// from the end-of-game stats, so pin it.
static_assert(GAME_OVER_MULTIPLAYER_MAX_PLAYERS_TRACKED == 8,
              "game-over stats must track the full eight-player roster");
...
**  memory smash. The XL brain states its facts so the host can ASSERT them at
**  dlopen and refuse a mismatched dylib loudly. Sizes are of the structs as
**  this dylib compiled them; MapStride is the engine's compile-time cell-array
**  width (MAP_CELL_W = 1024).
**
**  Fetched via the exported CNC3D_ABI_Facts(struct CNC3DAbiFactsStruct*).
**
*/
struct CNC3DAbiFactsStruct
{
    int AbiVersion;        // CNC_DLL_API_VERSION as compiled
    int MapStride;         // MAP_CELL_W: cells per row of the engine's cell array
    int MaxExportCells;    // MAX_EXPORT_CELLS as compiled into the fetch structs
    int MaxHouses;         // MAX_HOUSES rows in the per-house arrays
    int MaxPlayers;        // engine MAX_PLAYERS (multiplayer seats)
    int SizeofMapData;     // sizeof(CNCMapDataStruct)
    int SizeofPlayerInfo;  // sizeof(CNCPlayerInfoStruct)
    int SizeofShroudEntry; // sizeof(CNCShroudEntryStruct)
};
