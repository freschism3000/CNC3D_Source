...
COORDINATE UnitClass::Sort_Y(void) const
{
    ...
    if (IsTethered && *this == UNIT_HOVER) {
        return (Coord_Add(Coord, XY_Delta(0, -128)));
    }
    return (Coord_Add(Coord, XY_Delta(0, 128)));
}
...
void UnitClass::AI(void)
{
    ...
    */
    ...
    if (*this == UNIT_VICE && Map[Coord_Cell(Coord)].Land_Type() == LAND_TIBERIUM && Health_Ratio() < 0x0100
        && (Frame % 16) == 0) {
        Strength++;
        Mark(MARK_CHANGE);
    }
...
RadioMessageType UnitClass::Receive_Message(RadioClass* from, RadioMessageType message, int& param)
{
    ...
    */
    case RADIO_CAN_LOAD:
        if (Class->IsTransporter && How_Many() < Class->Max_Passengers() && from
...
ResultType UnitClass::Take_Damage(int& damage, int distance, WarheadType warhead, TechnoClass* source)
{
    ...
    */
    bool select =
        (Is_Selected_By_Player() && Is_Owned_By_Player()); // Fox for GlyphX multiplayer. ST - 4/16/2019 9:46AM
    ...
    */
    ...
    if (res == RESULT_DESTROYED) {
        ...
        */
        if (Health_Ratio() < 0x0080 && !IsAnimAttached && *this != UNIT_VICE && *this != UNIT_STEG && *this != UNIT_TREX
            && *this != UNIT_TRIC && *this != UNIT_RAPT) {
        ...
        */
        if (res == RESULT_HALF) {
            if (*this == UNIT_GUNBOAT) {
                AnimClass* anim =
                    new AnimClass(ANIM_ON_FIRE_MED, Coord_Add(Coord, XYP_Coord(Random_Pick(0, 16) - 8, -2)));
                if (anim)
                    anim->Attach_To(this);
            }
        }
        ...
        if (Class->IsChunkyShape) {
            ...
            */
            if (IsHarvesting && !PrimaryFacing.Is_Rotating() && !NavCom && !IsDriving) {
                ...
                shapenum =
                    32 + (((UnitClass::BodyShape[facing] + 2) / 4) * 4) + _hstage[Fetch_Stage() % sizeof(_hstage)];
            } else {
                ...
                if (Class->IsPieceOfEight) {
                    ...
                    if (facing)
                        shapenum = UnitClass::BodyShape[24 + facing];
                    if (IsDriving)
                        shapenum = (Fetch_Stage() % numshapes) + 16 + (shapenum * numshapes);
                    else if (IsFiring)
                        shapenum = (Fetch_Stage() % numshapes) + 80 + (shapenum * numshapes);
                } else {
        ...
        */
        if (*this == UNIT_HOVER && Is_Something_Attached()) {
            ...
            for (;;) {
                ...
                if (Map.Coord_To_Pixel(Coord_Add(Coord_Add(Coord, XY_Delta(-128, -128)), StoppingCoordAbs[counter++]), x1, y1)) {
...
BuildingClass* UnitClass::Find_Best_Refinery(void) const
{
    ...
    */
    if (TiberiumUnloadRefinery != NULL) {
        if (TiberiumUnloadRefinery->House == House && !TiberiumUnloadRefinery->IsInLimbo
...
bool UnitClass::Harvesting(void)
{
    ...
    */
    if (Target_Legal(NavCom) || !HarvestTimer.Expired() || IsDriving || IsRotating)
        return (true);
        ...
        if (*this == UNIT_GUNBOAT) {
            if (!Target_Legal(NavCom)) {
                if (PrimaryFacing == DIR_W) {
                    ...
                    Assign_Destination(
                        ::As_Target(XY_Cell(Map.MapCellX + Map.MapCellWidth, Cell_Y(Coord_Cell(Coord)))));
                }
                Set_Speed(255);
            }
            if (!Speed) {
        ...
        if (optr->IsWall) {
            if (Class->Primary != WEAPON_NONE) {
                WarheadTypeClass const* whead =
                    &Warheads[BulletTypeClass::As_Reference(Weapons[Class->Primary].Fires).Warhead];
    ...
    while (obj) {
        ...
        if (obj != this) {
            ...
            */
            ...
            bool is_moving = (obj->What_Am_I() == RTTI_INFANTRY || obj->What_Am_I() == RTTI_UNIT)
                        ...
                        if (unit->Unlimbo(coord, dir)) {
                            ...
                            */
                            if (*unit == UNIT_GUNBOAT) {
                                ...
                                unit->Commence();
                                unit->Assign_Destination(
                                    ::As_Target(XY_Cell(Map.MapCellX - 1, Cell_Y(Coord_Cell(unit->Coord)))));
                            }
