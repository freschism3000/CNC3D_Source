...
{
    if (IsUnloading) {
        return (Coord_Add(Coord, XY_Delta(0, 256)));
    }
    if (In_Radio_Contact() && IsTethered && Contact_With_Whom()->What_Am_I() == RTTI_UNIT) {
        return (Coord_Add(Coord, XY_Delta(0, 256)));
    }
    return (Coord_Add(Coord, XY_Delta(0, 48)));
}
...
int FootClass::Mission_Guard_Area(void)
{
    ...
    */
    ...
    if (!Target_Legal(NavCom)
        && (Distance(ArchiveTarget) > maxrange || (!Target_Legal(TarCom) && Distance(ArchiveTarget) > 0x0200))) {
        Assign_Target(TARGET_NONE);
...
ResultType FootClass::Take_Damage(int& damage, int distance, WarheadType warhead, TechnoClass* source)
{
    ...
    if (result != RESULT_NONE && Team) {
        ...
        if (result != RESULT_DESTROYED) {
            ...
            */
            if (source && !House->Is_Ally(source) && Techno_Type_Class()->Primary != WEAPON_NONE
...
void FootClass::Per_Cell_Process(bool center)
{
    ...
    */
    if (What_Am_I() != RTTI_UNIT || *((UnitClass*)this) != UNIT_GUNBOAT) {
        ...
        if (Target_Legal(TarCom)
            && (Mission == MISSION_RESCUE || Mission == MISSION_GUARD_AREA || Mission == MISSION_ATTACK
...
void FootClass::Death_Announcement(TechnoClass const* source) const
{
    ...
    if ((GameToPlay == GAME_GLYPHX_MULTIPLAYER && House->IsHuman)
        || (GameToPlay != GAME_GLYPHX_MULTIPLAYER && Is_Discovered_By_Player() || Is_Owned_By_Player())) { // ST
...
TARGET FootClass::Greatest_Threat(ThreatType method) const
{
    ...
    */
    ...
    if (Techno_Type_Class()->Secondary != WEAPON_NONE
        && BulletTypeClass::As_Reference(Weapons[Techno_Type_Class()->Secondary].Fires).IsAntiAircraft) {
        method = method | THREAT_AIR;
    }
...
bool FootClass::Can_Demolish(void) const
{
    ...
    case RTTI_AIRCRAFT:
        if (In_Radio_Contact() && Contact_With_Whom()->What_Am_I() == RTTI_BUILDING
            && *((BuildingClass*)Contact_With_Whom()) == STRUCT_REPAIR && Distance(Contact_With_Whom()) < 0x0080) {
