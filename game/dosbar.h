...
typedef struct
{
    ...
    int repair_disabled;      /* Repair->Disable() while BScan == 0  sidebar.cpp:508 */
    int sell_pressed;
    int sell_on;
    int sell_disabled;
    int map_disabled;         /* Zoom->Disable() when !IsRadarActive */
    int radar_active;
    int power_total;          /* drawn as the vertical power bar, in BAR PIXELS */
    int power_drain;          /* the drain marker's height, also in BAR PIXELS   */
    /* THE SAME TWO NUMBERS IN WATTS, and they are not interchangeable with the pixels
       above. power.cpp:425 Power_Height is a saturating curve (six equal fractions of
       what is left per 100 units), so a pixel ratio is NOT a watt ratio. 1995 draws the
       GEOMETRY from the pixels and picks the COLOUR from the watts:
           power.cpp  if (PlayerPtr->Drain > PlayerPtr->Power)     YELLOW
                      if (PlayerPtr->Drain > PlayerPtr->Power * 2) RED
       Comparing the pixel heights instead, as this did until v0.5.9, makes RED almost
       unreachable: the curve saturates, so a drain of twice the output is nowhere near
       twice the height. */
    int power_watts;
    int drain_watts;
    int nod;                  /* radar.cpp:348 RadarAnim is RADAR.<house suffix>:
                                 0 -> RADARGDI, 1 -> RADARNOD                     */
} DB_State;
...
/* The credits plate, tab.cpp:129 Draw_Credits_Tab + credits.cpp:112 Graphic_Logic.
 * It is NOT part of the sidebar column: at 320x200 it sits at x 160..239, directly
 * left of the sidebar tab, and the number is centred on x = width - 120 == 200. It
 * lands in the same 320x200 surface, so the caller can put it on screen from the
 * same texture. */
#define DB_CREDIT_TAB_X 160
#define DB_CREDIT_TEXT_X 200
