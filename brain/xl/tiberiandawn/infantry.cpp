...
void InfantryClass::Per_Cell_Process(bool center)
{
    ...
    */
    ...
    if (center && Mission == MISSION_ENTER && techno && Coord_Cell(Coord) == Coord_Cell(techno->Coord)
        ...
        if (House->IsHuman || GameToPlay != GAME_NORMAL) {
            ...
            if (sight) {
                Map.Sight_From(House,
                               Coord_Cell(Coord),
                               sight,
                               incremental); // Passed our house into Map.Sight_From since it now needs to know who it
                                             // is performing the action on behalf of. ST - 3/28/2019 2:55PM
            }
        }
    }
}
                    ...
                    if (!Basic_Path()) {
                        ...
                        if (Distance(NavCom) < 0x0280 && !IsTethered) {
                            ...
                            if (TryTryAgain) {
                                ...
                                // If we're trying to enter a transport we need to fail so others can try to enter. -
                                // LLL 4/17/2020
    ...
    */
    ...
    while (obj) {
        ...
        if (obj != this) {
            ...
            **	at a helipad.
            */
            if ((Mission != MISSION_CAPTURE && Mission != MISSION_SABOTAGE) || obj->What_Am_I() != RTTI_AIRCRAFT
    ...
    if (forced || Class->IsFraidyCat /*|| !(Random_Pick(1, 4) == 1)*/) {
        ...
        for (FacingType face = FACING_N; face < FACING_COUNT; face++) {
            ...
            if (Map.In_Radar(newcell) && Can_Enter_Cell(newcell) == MOVE_OK) {
                if (!Class->IsAvoidingTiberium || Map[newcell].Overlay == OVERLAY_NONE
...
TARGET InfantryClass::Greatest_Threat(ThreatType threat) const
{
    ...
    */
    ...
    case WEAPON_NONE:
        ...
        //
        // Crash here because INFANTRY_E7 has no primary weapon and WEAPON_NONE == -1, so we will reference a -ve index
        // into the Weapons array. Assume original intent was to fall out of the switch and call the base class
        // Greatest_Threat, so adding a break here. ST - 4/24/2019 11:01AM
        //
    ...
    if (*this == INFANTRY_RAMBO) {
        ...
        if (Class->IsCivilian) {
            if (*this == INFANTRY_MOEBIUS) {
                static VocType _response[] = {VOC_YES, VOC_COMMANDER, VOC_HELLO, VOC_HMMM};
                response = _response[Sim_Random_Pick(0, (int)(sizeof(_response) / sizeof(_response[0])) - 1)];
            } else {
                if (Class->IsFemale) {
            ...
            static VocType _response[] = {
                VOC_ACKNOWL, VOC_REPORT, VOC_REPORT, VOC_YESSIR, VOC_YESSIR, VOC_READY, VOC_AWAIT};
            response = _response[Sim_Random_Pick(0, (int)(sizeof(_response) / sizeof(_response[0])) - 1)];
        }
    }
    if (AllowVoice) {
...
void InfantryClass::Response_Move(void)
{
    ...
    VocType response;
    if (*this == INFANTRY_RAMBO) {
        static VocType _response[] = {VOC_RAMBO_UGOTIT, VOC_RAMBO_ONIT, VOC_RAMBO_NOPROB};
        response = _response[Sim_Random_Pick(0, (int)(sizeof(_response) / sizeof(_response[0])) - 1)];
    } else {
        if (Class->IsCivilian) {
            if (*this == INFANTRY_MOEBIUS) {
            ...
            static VocType _response[] = {
                VOC_MOVEOUT, VOC_MOVEOUT, VOC_MOVEOUT, VOC_ROGER, VOC_RIGHT_AWAY, VOC_UGOTIT, VOC_AFFIRM, VOC_AFFIRM};
            response = _response[Sim_Random_Pick(0, (int)(sizeof(_response) / sizeof(_response[0])) - 1)];
        }
    }
    if (AllowVoice) {
...
void InfantryClass::Response_Attack(void)
{
    ...
    VocType response;
    if (*this == INFANTRY_RAMBO) {
        static VocType _response[] = {VOC_RAMBO_NOPROB, VOC_RAMBO_UGOTIT, VOC_RAMBO_NOPROB, VOC_RAMBO_ONIT};
        response = _response[Sim_Random_Pick(0, (int)(sizeof(_response) / sizeof(_response[0])) - 1)];
    } else {
        if (Class->IsCivilian) {
            if (Class->IsFemale) {
            ...
            static VocType _response[] = {
                VOC_RIGHT_AWAY, VOC_AFFIRM, VOC_AFFIRM, VOC_UGOTIT, VOC_NO_PROB, VOC_YESSIR, VOC_YESSIR, VOC_YESSIR};
            response = _response[Sim_Random_Pick(0, (int)(sizeof(_response) / sizeof(_response[0])) - 1)];
        }
    }
...
ActionType InfantryClass::What_Action(ObjectClass* object) const
{
    ...
    */
    if (House->Is_Ally(object) && Is_Owned_By_Player() && // Changed for multiplayer. ST - 3/13/2019 5:37PM
        // IsOwnedByPlayer &&
        object->Is_Techno() &&
        // IsOwnedByPlayer &&
        ((InfantryClass*)this)->Transmit_Message(RADIO_CAN_LOAD, (TechnoClass*)object) == RADIO_ROGER) {
        //	if (object->Owner() == Owner() && object->What_Am_I() == RTTI_UNIT && ((UnitClass
        //*)object)->Class->IsTransporter && ((UnitClass *)object)->How_Many() < 5) {
        action = ACTION_ENTER;
    }

    if (Class->IsCapture && action == ACTION_ATTACK) {
        if (object->Owner() != Owner()
            && ((object->What_Am_I() == RTTI_AIRCRAFT && ((AircraftClass*)object)->Pip_Count() == 0
                 && *((AircraftClass*)object) == AIRCRAFT_TRANSPORT)
                || (object->What_Am_I() == RTTI_BUILDING && ((BuildingClass*)object)->Can_Capture()))) {
