...
public:
    ...
    */
    ...
    /*
    ** A flag to indicate that this object was recently created. Since an object's allocation is just a matter of
    *whether
    ** the IsActive flag is set, during a logic frame an object with a given ID could be 'deleted' then reallocated
    ** as a different type of object in a different location. This flag lets us know that this happened. ST - 8/19/2019
    *5:33PM
    */
    ...
    */
    ...
    /* XL: the CELL overload died with the widening -- CELL is int now, the
       same scalar as TARGET, so it cannot coexist with Direction(TARGET).
       The two former callers spell the global cell-resolution form out
       (DIVERGENCE.md, types). */
    ...
    /* XL: Distance(CELL) died with the widening for the same reason as
       Direction(CELL) above; it had no callers at the fork. */
