...
{
    return XY_Cell(Coord_X(coord) >> 8, Coord_Y(coord) >> 8);
}
...
short const* Coord_Spillage_List(COORDINATE coord, int maxsize)
{
    static short const _MoveSpillage[(int)FACING_COUNT + 1][5] = {
        ...
        //		{0, -MAP_CELL_W, -(MAP_CELL_W-1), 1, MAP_CELL_W+1, MAP_CELL_W, MAP_CELL_W-1, -1, -(MAP_CELL_W+1),
        //REFRESH_EOL}
    };
    ...
    */
    if (maxsize > ICON_PIXEL_W * 2) {
        static short const _gigundo[] = {-((2 * MAP_CELL_W) - 2), -((2 * MAP_CELL_W) - 1),
...
{
    /* XL: the classic short locals were the axis-truncation audit's poster
       child (docs/design-xl-brain.md, precisionAndFeel); positions cap at
       2^20 leptons per axis so int is exact. */
    int x = Coord_X(start);
    int y = Coord_Y(start);
...
void Move_Point(int& x, int& y, register DirType dir, unsigned short distance)
{
    ...
    /* XL: common's calcx/calcy return their fixed-point product TRUNCATED to an
       unsigned 16-bit pattern (the classic axis width), and the classic short
       x/y wrapped that back into a signed delta for free. Widened axes need the
       sign restored explicitly, and common/ is frozen byte-identical to
       vanilla's (tools/check-xl-common.sh), so the cast lives here. Exact for
       |delta| < 32768 leptons (128 cells) per call -- per-tick motion and every
       scatter distance are orders of magnitude below it. */
    x += (short)calcx((char)CosTable[dir], distance);
    y += (short)calcy((char)SinTable[dir], distance);
}
