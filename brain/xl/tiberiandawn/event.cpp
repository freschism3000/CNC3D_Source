...
const char* EventClass::EventNames[EventClass::LAST_EVENT] = {
    "EMPTY",         "ALLY",          "MEGAMISSION", "IDLE",    "SCATTER", "DESTRUCT",     "DEPLOY",
    "PLACE",         "OPTIONS",       "GAMESPEED",   "PRODUCE", "SUSPEND", "ABANDON",      "PRIMARY",
    "SPECIAL_PLACE", "EXIT",          "ANIMATION",   "REPAIR",  "SELL",    "SPECIAL",      "FRAMESYNC",
    "MESSAGE",       "RESPONSE_TIME", "FRAMEINFO",   "ARCHIVE", "TIMING",  "PROCESS_TIME",
};
...
{
    /*
    ** XL: this ctor also carries the former (int id, CELL cell) SPECIAL_PLACE
    ** ctor -- CELL widened to int and the signatures merged. Data.Special must
    ** therefore alias Data.NavCom exactly, which the asserts pin; Execute()
    ** reads through the member the event TYPE names either way.
    */
    static_assert(sizeof(Data.Special) == sizeof(Data.NavCom), "Special must alias NavCom");
    static_assert(sizeof(Data.Special.ID) == sizeof(Data.NavCom.Whom), "ID must alias Whom");
    static_assert(sizeof(Data.Special.Cell) == sizeof(Data.NavCom.Where), "Cell must alias Where");
    ID = Houses.ID(PlayerPtr);
    Type = type;
    Frame = ::Frame;
...
{
    /*
    ** XL: this ctor also carries the former (RTTIType, CELL) PLACE ctor --
    ** CELL widened to int and the signatures merged. Data.Place must
    ** therefore alias Data.Specific exactly, which the asserts pin; the
    ** classic engine already used this pun when sidebar placement passed -1
    ** for a PLACE event through this very ctor.
    */
    static_assert(sizeof(Data.Place) == sizeof(Data.Specific), "Place must alias Specific");
    static_assert(sizeof(Data.Place.Cell) == sizeof(Data.Specific.ID), "Cell must alias ID");
    ID = Houses.ID(PlayerPtr);
    Type = type;
    Frame = ::Frame;
 ...
 *   05/18/1995 JLB : Created.                                                                 *
 *=============================================================================================*/
/* XL: the (RTTIType, CELL) PLACE ctor merged into the (RTTIType, int) ctor
   above -- see the layout asserts there. */
 ...
 *   05/18/1995 JLB : Created.                                                                 *
 *=============================================================================================*/
/* XL: the (int, CELL) SPECIAL_PLACE ctor merged into the (TARGET, TARGET)
   ctor -- Data.Special aliases Data.NavCom field-for-field, asserted there. */
